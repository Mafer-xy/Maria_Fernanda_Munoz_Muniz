---
titulo: "Sesión 5 — Mecanismos"
fecha: 2026-09-18
autor: "María Fernanda Muñoz Muñiz"
equipo: "Fernanda Muñoz y Mario Osorio"
estado: borrador #borrador | completa 
---

# Mecanismos

## Qué debía lograr hoy
 

- [✅] Entender cómo los mecanismos transforman el movimiento (velocidad ↔ par, rotación ↔ traslación, continuo ↔ intermitente)
- [✅] calcular relaciones de transmisión simples 
- [❌] Aplicarlo al tren motriz del carro — tocando mecanismos reales en un taller de estaciones.

## Qué usé

- Modelos impresos 3D de diferentes mecanismos:
    * Engrane Interno Helicoidal
    * Cónico Espiral
    * Espiral de Arquímedes
    * Reductor Cicloidal
    * Sinfín Multi-hilo + Corona
    * Engrane Intermitente
    * Cruz de Ginebra
    * Junta Carbán (Universal)
    * Obturador de láminas (leaf shutter)
    * Diferencial

## Qué hice y qué pasó (evidencia)
*2–4 fotos o capturas TUYAS, cada una con un pie de foto de una línea diciendo qué muestra.
Si mediste algo, va la tabla — la tabla ES la evidencia.*

![Pie de foto: qué muestra esta imagen](img/sesionN_1.jpg)

| Estación | ¿Qué transforma? | relación *i* estimada | ¿Reversible o autobloqueante? |¿Dónde lo has visto en la vida real?|¿Donde serviría en el carro o en un proyecto tuyo?|
| --- | --- | --- | --- | --- | --- |
|A. Diferencial  |  |  |  |  |  |  |
| --- | --- | --- | --- | --- | --- |
|B. Cicloidal |  |  |  |  |  |  |
| --- | --- | --- | --- | --- | --- |
|C. Cardán |  |  |  |  |  |
| --- | --- | --- | --- | --- | --- |
|D. Obturador |  |  |  |  |  |
| --- | --- | --- | --- | --- | --- |
|E1. Cremallera |  |  |  |  |  |
| --- | --- | --- | --- | --- | --- |
|E1. Ginebra |  |  |  |  |  |
| --- | --- | --- | --- | --- | --- |
|E2. Espiral |  |  |  |  |  |
| --- | --- | --- | --- | --- | --- |
|E2. Sinfín multi-hilo + corona |  |  |  |  |  |
| --- | --- | --- | --- | --- | --- |

## Qué falló y cómo lo resolví

- **Síntoma:** ...
- **Cómo lo encontré:** ...
- **Solución:** ...

## Qué aprendí
*3 a 5 líneas, con tus palabras. No es resumen del tema: es qué entendiste TÚ que antes no.*

## Siguiente paso
Analizar cada engranaje y encontrar los mas optimos para realizar los carros del proyecto final